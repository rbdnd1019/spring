					...
					<div class="card-body">
                     	<form method="post">
							...
							<sec:authorize access="isAuthenticated() && principal.username == #board.writer">
								<a class="btn btn-secondary btn-sm" href="modify${cri.params}&bno=${board.bno}">수정</a>
							</sec:authorize>
						</form>
						</div>
                    </div>
			...
			/* 	$(document).ajaxSend(function(e, xhr) {
				...
				$(function() {
					...
					function getReplyStr(reply){
						...
						str += '	<small class="float-right">' + replyService.displayTime(reply.replyDate) + '</small>';
				...
				// 글 등록
				/*
				replyService.add({bno:bno, reply:"get.jsp 작성 테스트", replyer:"작성자"},
						function(result) {
					alert("글 등록 성공");
					console.log(result);
				});
				*/
				...
				/*
				replyService.remove(363856, function(result){
					alert("글 삭제 성공")
					console.log(result);
				}) 
				*/
				...
				/* 
				replyService.get(363855, function(result){
					alert("글 조회 성공")
					console.log(result);
				})
				*/
				...
				// 글 수정
				/*
				replyService.update({rno:363856, reply:"get.jsp 수정 테스트"}, function(result){
					alert("글 수정 성공");
					console.log(result);
				})
				*/			
